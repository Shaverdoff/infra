...
where:
sentry.company.ru– sentry url
company - organization slug in sentry
Client Protocol – saml
...
# go to realm settings and copy Endpoints for SAML2 from url on that page
...
Go to KEYCLOAK/CLIENTS/YOUR CLIENT / SAML KEYS => GENERATE new keys
ERROR
VerificationException: SigAlg was null
Выключил в кейклоке в настройках клиента
Client Signature Required 

# Authentication error: SAML SSO failed, The Assertion must include an AuthnStatement element
В кейклоак / клиент / включить
Include AuthnStatement
Authentication error: SAML SSO failed, No Signature found. SAML Response rejected
```